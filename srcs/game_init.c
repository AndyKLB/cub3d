...
/*   Updated: 2025/01/30 14:13:03 by ankammer         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	xpm_to_img(t_cub3d *cub3d)
{
	...
	cub3d->img->wall_so = mlx_xpm_file_to_image(cub3d->mlx,
			cub3d->south_element, &cub3d->img->height, &cub3d->img->width);
	if (!cub3d->img->wall_so)
		return (free(cub3d->img->wall_no), 1);
	cub3d->img->wall_ea = mlx_xpm_file_to_image(cub3d->mlx, cub3d->east_element,
			&cub3d->img->height, &cub3d->img->width);
	if (!cub3d->img->wall_ea)
		return (free(cub3d->img->wall_no), free(cub3d->img->wall_so), 1);
	cub3d->img->wall_we = mlx_xpm_file_to_image(cub3d->mlx, cub3d->west_element,
			&cub3d->img->height, &cub3d->img->width);
...
void	draw_map(t_cub3d *cub3d)
{
	...
	while (cub3d->maps[++y])
	{
		...
		while (cub3d->maps[y][++x])
		{
			...
			else if (cub3d->maps[y][x] == '0')
			// comment dessiner le plafond et le sol ?
			{
