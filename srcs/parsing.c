...
/*   Updated: 2025/01/23 11:57:24 by ankammer         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void check_wall(t_cub3d *cub3d)
{
	...
	while (cub3d->maps[j])
	{
		...
		else
		{
			...
			{
				printf("i = %d et j = %d\n", i, j);
				printf(" len j + 1 = %lu et len j - 1 = %lu\n", ft_strlen(cub3d->maps[j - 1]) - 1, ft_strlen(cub3d->maps[j + 1]) - 1);
				if ((cub3d->maps[j][i] == ' ' && i > (int)ft_strlen(cub3d->maps[j - 1]) - 1) || (cub3d->maps[j][i] == ' ' && i > (int)ft_strlen(cub3d->maps[j + 1]) - 1))
				{
					printf("i = %d et j = %d\n", i, j);
					printf(" len j + 1 = %lu et len j - 1 = %lu\n", ft_strlen(cub3d->maps[j - 1]) - 1, ft_strlen(cub3d->maps[j + 1]) - 1);
				}
